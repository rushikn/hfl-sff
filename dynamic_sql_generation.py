...
business_term_mapping = {
    "financial":"[[Financial year is defined by its ending March; for example, financial year 2024 spans from April 2023 to March 2024, and financial year 2025 spans from April 2024 to March 2025 — the year is determined by the ending March.]]",
...
"livo": "[livo means strictly you have to map like this: FROM DW.fSales WHERE MaterialCode IN ('30769', '30704', '30706')]",
    ...
    "mom":"month on month",
    "tso":"TSO",
"sale": "[sale means SUM(SalesQuantity) / number_of_days_in_time_period, where number_of_days_in_time_period = total full days in the query date range. But if the query expects day-wise output (i.e., each day's sale shown separately), then do NOT divide by number_of_days — just use SUM(SalesQuantity) * 1.0 per day as each row already represents a single day. Also, if the user query is only asking for overall sale (without mentioning any product), do NOT include SalesUnit in the output. Only when user explicitly mentions any product from any ProductHeirachy or material group, you MUST include this also: MAX(f.SalesUnit) AS SalesUnit — regardless of any other filters like location or time period.]",
    "year on year":"YoY",
    "day on day":"DoD",
    "mom":"MoM",
    "yoy":"YoY",
...
- Output queries that are complete, correct, and production-ready.
    ...
    MAIN LOGIC:
    eg 1) what is the seasonality for the financial year 2024 ? SQL: WITH MonthlyAvg AS (SELECT CASE WHEN MONTH(f.BillingDate) >= 4 THEN YEAR(f.BillingDate) + 1 ELSE YEAR(f.BillingDate) END AS FinancialYear, MONTH(f.BillingDate) AS MonthNumber, DATENAME(MONTH, f.BillingDate) AS MonthName, EOMONTH(f.BillingDate) AS MonthEndDate, CAST(SUM(f.SalesQuantity) * 1.0 / DAY(EOMONTH(f.BillingDate)) AS DECIMAL(18, 2)) AS AvgDailySales FROM DW.fSales f WHERE f.BillingDate >= '2023-04-01' AND f.BillingDate < '2024-04-01' AND f.ProductHeirachy1 NOT IN ('Feed', 'Feed Supplement', 'Veterinary Medicines', 'Water', 'HERITA LIV', 'NULL', 'others') GROUP BY MONTH(f.BillingDate), DATENAME(MONTH, f.BillingDate), EOMONTH(f.BillingDate), CASE WHEN MONTH(f.BillingDate) >= 4 THEN YEAR(f.BillingDate) + 1 ELSE YEAR(f.BillingDate) END), OverallAvg AS (SELECT FinancialYear, CAST(SUM(AvgDailySales) / 12 AS DECIMAL(18, 2)) AS Avg FROM MonthlyAvg WHERE FinancialYear = 2024 GROUP BY FinancialYear), Pivoted AS (SELECT m.FinancialYear AS [Year], MAX(CASE WHEN m.MonthNumber = 4 THEN m.AvgDailySales END) AS April, MAX(CASE WHEN m.MonthNumber = 4 THEN m.AvgDailySales - o.Avg END) AS [April Delta], MAX(CASE WHEN m.MonthNumber = 5 THEN m.AvgDailySales END) AS May, MAX(CASE WHEN m.MonthNumber = 5 THEN m.AvgDailySales - o.Avg END) AS [May Delta], MAX(CASE WHEN m.MonthNumber = 6 THEN m.AvgDailySales END) AS June, MAX(CASE WHEN m.MonthNumber = 6 THEN m.AvgDailySales - o.Avg END) AS [June Delta], MAX(CASE WHEN m.MonthNumber = 7 THEN m.AvgDailySales END) AS July, MAX(CASE WHEN m.MonthNumber = 7 THEN m.AvgDailySales - o.Avg END) AS [July Delta], MAX(CASE WHEN m.MonthNumber = 8 THEN m.AvgDailySales END) AS August, MAX(CASE WHEN m.MonthNumber = 8 THEN m.AvgDailySales - o.Avg END) AS [August Delta], MAX(CASE WHEN m.MonthNumber = 9 THEN m.AvgDailySales END) AS September, MAX(CASE WHEN m.MonthNumber = 9 THEN m.AvgDailySales - o.Avg END) AS [September Delta], MAX(CASE WHEN m.MonthNumber = 10 THEN m.AvgDailySales END) AS October, MAX(CASE WHEN m.MonthNumber = 10 THEN m.AvgDailySales - o.Avg END) AS [October Delta], MAX(CASE WHEN m.MonthNumber = 11 THEN m.AvgDailySales END) AS November, MAX(CASE WHEN m.MonthNumber = 11 THEN m.AvgDailySales - o.Avg END) AS [November Delta], MAX(CASE WHEN m.MonthNumber = 12 THEN m.AvgDailySales END) AS December, MAX(CASE WHEN m.MonthNumber = 12 THEN m.AvgDailySales - o.Avg END) AS [December Delta], MAX(CASE WHEN m.MonthNumber = 1 THEN m.AvgDailySales END) AS January, MAX(CASE WHEN m.MonthNumber = 1 THEN m.AvgDailySales - o.Avg END) AS [January Delta], MAX(CASE WHEN m.MonthNumber = 2 THEN m.AvgDailySales END) AS February, MAX(CASE WHEN m.MonthNumber = 2 THEN m.AvgDailySales - o.Avg END) AS [February Delta], MAX(CASE WHEN m.MonthNumber = 3 THEN m.AvgDailySales END) AS March, MAX(CASE WHEN m.MonthNumber = 3 THEN m.AvgDailySales - o.Avg END) AS [March Delta], o.Avg FROM MonthlyAvg m JOIN OverallAvg o ON m.FinancialYear = o.FinancialYear WHERE m.FinancialYear = 2024 GROUP BY m.FinancialYear, o.Avg) SELECT * FROM Pivoted;

    IMPORTANT: "If the user mentions any specific product (like Milk, Ice Cream, etc.), then apply the filter d.[Sales Offcie Type] = 'sales Office' by joining DW.dSalesOfficeMaster; if no product is mentioned, skip this filter and no join is needed."
